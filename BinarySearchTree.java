...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    protected void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        // Checking if the child provided is a right child or left child
        if (child.isRightChild() && parent.down[1] == child) {
            // Saving the parent of the given parent node as a reference
            Node<T> grandparent = parent.up;
            // Performing a left rotation:
            parent.down[1] = child.down[0]; // Update parent's right subtree to child's left subtree
            if (child.down[0] != null) {
                child.down[0].up = parent; // Update child's left subtree's parent to be the parent
            }
            child.down[0] = parent; // Set child's left child to be the parent
            parent.up = child; // set parent's parent to be the child

            if (grandparent != null) {
                if (parent == grandparent.down[0]) {
                    grandparent.down[0] = child; // Update grandparent's child reference
                } else {
                    grandparent.down[1] = child; // Update grandparent's child reference
                }
                child.up = grandparent; // Set child's parent to be the grandparent
            } else {
                // If parent was the root, update the root reference
                this.root = child;
                child.up = null;
            }
        } else if (!child.isRightChild() && parent.down[0] == child)  {
            // right rotation (symmetric to the left rotation)
            Node<T> grandparent = parent.up;
            // Performing a right rotation:
            parent.down[0] = child.down[1]; // Update parent's left subtree to child's right subtree
            if (child.down[1] != null) {
                child.down[1].up = parent; // Update child's right subtree's parent to be the parent
            }
            child.down[1] = parent; // Set child's right child to the parent
            parent.up = child; // set parent's parent to be the child
            ...
            // Invalid relationship between child and parent
    ...
    /**
     * This test is to check if the right rotation works properly or not from the root.
     *
    ...
    public static boolean test1() {
        ...
        bst.rotate(two, four);
        // Verify the tree structure after left rotation
        if (!bst.toLevelOrderString().equals("[ 2, 1, 4, 3, 5 ]") ) {
            return false;
        }
        if (!bst.toInOrderString().equals("[ 1, 2, 3, 4, 5 ]")) {
            return false;
        }
        return true;
    }
    ...
    /**
     * This test is to check if the left rotation works properly or not
     *
     * @return true if all the test passes.
     */
    public static boolean test2() {
        ...
        Node<Integer> nine = three.down[1];
        bst.rotate(nine, three); // rotating nine and three
        if (!bst.toLevelOrderString().equals("[ 9, 3, 10, 2, 5 ]")) { //9 becomes the roo
            return false;
        }
        if (!bst.toInOrderString().equals("[ 2, 3, 5, 9, 10 ]")) {
            return false;
        }
        return true;
    }
    ...
    /**
     * This test is to check if the right rotation works properly when their are only 2 nodes
     *
     * @return true if all the test passes.
     */
    public static boolean test3() {
        ...
        bst.rotate(four, six); // rotating two and four

        if (!bst.toLevelOrderString().equals("[ 4, 6, 7 ]")) {
            return false;
        }
        if (!bst.toInOrderString().equals("[ 4, 6, 7 ]")) {
            return false;
        }
        return true;
    }
    ...
    /**
     * check if the method throws an exception when the child and parent are not related.
     *
    ...
    public static boolean test4() {
        ...
        try {
            ...
            bst.insert(3);
            bst.insert(1);
            Node<Integer> four = bst.root.down[0];
            Node<Integer> seven = bst.root.down[1];
            bst.rotate(four, seven); // rotating two and four
            return false;
